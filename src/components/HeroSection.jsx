...
export function HeroSection() {
  const { images } = data
  return <section className="hero cinematic-hero" style={{ '--hero-image': `url(${images.hero.src})`, '--hero-position': images.hero.position, '--hero-mobile-position': images.hero.mobilePosition }} role="img" aria-label={images.hero.alt}><motion.div className="hero-content" initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1.1, ease: [0.22, 1, 0.36, 1] }}><div className="hero-monogram">S · C</div></motion.div><a href="#countdown" className="scroll-cue" aria-label="Deslize para descobrir o convite"><span>Deslize para descobrir/ver o convite</span><FiArrowDown /></a></section>
}
