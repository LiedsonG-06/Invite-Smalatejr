...
export function GiftSection() {
  ...
  return <section className="section gifts"><Reveal><div className="simple-heading"><h2>Presentes</h2></div><p className="section-intro gift-intro">A vossa presença é o nosso maior presente. Contudo, para quem desejar oferecer-nos uma lembrança, disponibilizamos as seguintes opções.</p><div className="gift-actions"><a className="outline-btn" href={giftContactUrl} target="_blank" rel="noreferrer"><FiMessageCircle /> Contactar Responsavel</a><button className="outline-btn" type="button" onClick={() => setModalOpen(true)}><FiCreditCard /> Detalhes Bancarios</button></div></Reveal><TransferModal open={modalOpen} onClose={closeModal} /></section>
}
