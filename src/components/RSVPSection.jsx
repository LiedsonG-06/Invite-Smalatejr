...
export function RSVPSection() {
  ...
  const update = ({ target }) => { const next = { ...form, [target.name]: target.value }; if (target.name === 'confirmacao' && target.value === 'Não') { next.numeroAcompanhantes = 0; next.nomesAcompanhantes = '' } if (target.name === 'numeroAcompanhantes' && Number(target.value) === 0) { next.nomesAcompanhantes = '' } setForm(next); setStatus({ type: '', message: '' }) }
  const submit = async (event) => {
    ...
    const guestCount = attending ? Number(form.numeroAcompanhantes) : 0
    if (attending && (!Number.isInteger(guestCount) || guestCount < 0)) { setStatus({ type: 'error', message: 'O número de acompanhantes deve ser zero ou um número positivo.' }); return }
    const guestNames = attending && guestCount > 0
      ? form.nomesAcompanhantes.split(/[,\n]/).map((name) => name.trim()).filter(Boolean)
      : []
    if (attending && guestNames.length !== guestCount) { setStatus({ type: 'error', message: 'Informe o nome de todos os acompanhantes.' }); return }
    if (!RSVP_ENDPOINT) { setStatus({ type: 'error', message: 'A confirmação online ainda precisa de ser configurada pelos noivos.' }); return }
    const payload = { nomeCompleto: form.nomeCompleto.trim(), telefone: form.telefone.trim(), confirmacao: form.confirmacao, numeroAcompanhantes: guestCount, nomesAcompanhantes: guestNames, mensagem: form.mensagem.trim(), dataSubmissao: new Date().toISOString() }
